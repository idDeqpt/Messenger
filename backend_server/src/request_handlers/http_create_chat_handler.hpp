#ifndef CREATE_CHAT_HTTP_HANDLER_HPP
#define CREATE_CHAT_HTTP_HANDLER_HPP
	...
	net::HTTPResponse create_chat(net::HTTPRequest request)
	{
		...
		if (request.start_line[0] == "POST")
		{
			...
			else
			{
				...
				std::shared_ptr<db::DataBuffer> result_users = db::exec("SELECT id FROM users WHERE username = \"" + chat_name + "\";");
				if ((chat_type == "user") && (result_users->size() != 1))
				{
				...
				else
				{
					...
					if ((chat_type == "user") && (user_id != result_users->back()["id"]))
					{
						db::exec("INSERT INTO chat_members (chat_id, user_id)\
								  VALUES (" + result->back()["id"] + ", " + result_users->back()["id"] + ");");
					}
...
#endif //CREATE_CHAT_HTTP_HANDLER_HPP