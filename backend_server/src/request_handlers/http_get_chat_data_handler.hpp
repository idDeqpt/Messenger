			...
			else
			{
				...
				for (unsigned int i = 0; i < chat_members_data->size(); i++)
				{
					...
					if (chat_members_data->at(i)["has_profile_photo"] == "1")
						file.open(db::res_directory + "resources/profile_photos/" + chat_members_data->at(i)["user_id"] + ".png", std::ios::binary);
					else
						file.open(db::res_directory + "resources/profile_photos/default.png", std::ios::binary);
